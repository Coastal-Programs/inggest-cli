...
func TestListApps(t *testing.T) {
	...
	if apps[0].URL != "https://example.com/api/inngest" {
...
func TestGetApp(t *testing.T) {
	...
	if app.Functions[0].Name != "Send Email" {
	...
	if app.Functions[0].Triggers[0].Type != "event" {
		t.Errorf("expected trigger type 'event', got %q", app.Functions[0].Triggers[0].Type)
	}
	if app.Functions[0].Triggers[0].Value != "user/signup" {
